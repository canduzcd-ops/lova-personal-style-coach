...
export const analyzeImage = async (base64Image: string): Promise<AnalyzedItem | null> => {
  try {
    ...
    if (response.text) {
      return JSON.parse(response.text) as AnalyzedItem;
    }
    return null;
...
export const rateOutfit = async (base64Image: string): Promise<Omit<StyleRating, 'image' | 'date'> | null> => {
  try {
    ...
    if (response.text) {
      return JSON.parse(response.text) as Omit<StyleRating, 'image' | 'date'>;
    }
    return null;
...
export const analyzeFaceForMakeup = async (base64Image: string): Promise<MakeupAnalysis | null> => {
  try {
    ...
    if (response.text) {
      return JSON.parse(response.text) as MakeupAnalysis;
    }
    return null;
...
export const generatePersonalizedTips = async (user: UserProfile, wardrobe: WardrobeItem[]): Promise<StyleTipResult | null> => {
  try {
    ...
    const itemsSummary = wardrobe.map(item => 
        `${item.color} ${item.name} (${item.aiTags?.fabric || 'unknown fabric'}, ${item.aiTags?.aesthetic?.join(' ') || ''})`
    ).join(', ');
    ...
    if (response.text) {
      return JSON.parse(response.text) as StyleTipResult;
    }
    return null;
