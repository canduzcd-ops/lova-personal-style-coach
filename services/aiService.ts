...
const logGatewayError = (path: string, attempt: number, error: unknown) => {
...
async function analyzeImageDirectly(base64Image: string, retryCount = 0): Promise<AnalyzedItem | null> {
  if (!geminiApiKey) {
...
Only output the JSON object, nothing else.
  ...
  try {
    ...
    const parsed = JSON.parse(cleanedText) as AnalyzedItem;
...
async function generateOutfitDirectly(user: UserProfile, wardrobe: WardrobeItem[]): Promise<SuggestionResult | null> {
  if (!geminiApiKey) {
    console.error('[aiService] Gemini API key not configured');
    return null;
  }
...
`.trim();
  ...
  try {
    ...
    const outfitWithImages = {
      ...parsed.outfit,
      items: parsed.outfit.items.map((item: any) => {
        const matchedItem = wardrobe.find(w => 
    ...
    return {
      ...
      additionalTips: (parsed.additionalTips || []).map((tip: any, idx: number) => ({
        ...tip,
