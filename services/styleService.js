...
const calculateCompatibility = (item1, item2, targetStyle) => {
    let score = 0;
    const neutrals = ['Siyah', 'Beyaz', 'Gri', 'Bej', 'Antrasit', 'Ekru', 'Krem'];
    if (item1.color === item2.color)
        score += 3;
    else if (neutrals.includes(item1.color) || neutrals.includes(item2.color))
        score += 2;
    ...
    if (dna) {
        ...
        if (i1Matches && i2Matches)
            score += 5;
        else if (i1Matches || i2Matches)
            score += 2;
    }
    return score;
};
// --- STATIC TIPS ---
export const generateStaticStyleTips = (userStyles) => {
    const dominantStyleId = userStyles[Math.floor(Math.random() * userStyles.length)] || 'minimal';
    const dominantStyleLabel = STYLE_OPTIONS.find((s) => s.id === dominantStyleId)?.label || i18n.t('style.fallbackStyle', 'Modern');
    return {
...
export const generateSmartStyleTips = async (user, wardrobe) => {
    try {
...
export const generateSmartOutfit = async (user, wardrobe, anchorItem) => {
    if (anchorItem) {
...
export const generateOfflineSuggestion = (userProfile, wardrobe) => {
    ...
    const selectedItems = [];
    const dna = STYLE_DNA[targetStyle];
    const scoreItemForStyle = (item) => {
        let s = 0;
        if (item.aiTags?.style?.includes(targetStyle))
            s += 10;
        if (dna) {
            if (dna.colors.includes(item.color))
                s += 3;
            if (dna.fabrics.includes(item.aiTags?.fabric || ''))
                s += 4;
            if (dna.keywords.some((k) => item.name.toLowerCase().includes(k)))
                s += 2;
        }
    ...
    if (useDress) {
        ...
        if (bottoms.length > 0 && selectedItems.length > 0) {
            ...
            const sortedBottoms = [...bottoms].sort((a, b) => calculateCompatibility(heroTop, b, targetStyle) - calculateCompatibility(heroTop, a, targetStyle));
    ...
    if (outers.length > 0 && selectedItems.length > 0 && Math.random() > 0.3) {
        ...
        const sortedOuters = [...outers].sort((a, b) => calculateCompatibility(baseItem, b, targetStyle) - calculateCompatibility(baseItem, a, targetStyle));
    ...
    return {
        ...
        additionalTips: [
            ...
            },
            {
                ...
                desc: i18n.t('style.outfit.tips.tipSilhouetteDesc', 'Balance loose and fitted pieces to keep proportions clean.'),
                icon: Scissors,
            },
        ],
    };
};
export const generateSuggestionFromImage = (userProfile, wardrobe, targetItem) => {
