...
export const outfitHistoryService = {
    async addOutfit(userId, payload) {
        try {
            ...
            // Update cache
            const cachedOutfits = await cacheService.get(CACHE_KEYS.OUTFIT_HISTORY) || [];
            const newEntry = {
                id: docRef.id,
                userId,
                outfit: payload.outfit,
    ...
    async setFeedback(docId, userId, liked) {
        try {
            ...
            const cachedOutfits = await cacheService.get(CACHE_KEYS.OUTFIT_HISTORY);
            if (cachedOutfits) {
                const updated = cachedOutfits.map(item => item.id === docId ? { ...item, liked, feedbackAt: new Date().toISOString() } : item);
    ...
    async setFavorite(docId, userId, isFavorite) {
        try {
            const ref = db.collection(COLLECTION).doc(docId);
            const snap = await ref.get();
            if (!snap.exists)
            ...
            const cachedOutfits = await cacheService.get(CACHE_KEYS.OUTFIT_HISTORY);
            if (cachedOutfits) {
                const updated = cachedOutfits.map(item => item.id === docId ? { ...item, isFavorite } : item);
                await cacheService.set(CACHE_KEYS.OUTFIT_HISTORY, updated);
    ...
    async setCollectionTag(docId, userId, collectionTag) {
        try {
            const ref = db.collection(COLLECTION).doc(docId);
            const snap = await ref.get();
            if (!snap.exists)
            ...
            const cachedOutfits = await cacheService.get(CACHE_KEYS.OUTFIT_HISTORY);
            if (cachedOutfits) {
                const updated = cachedOutfits.map(item => item.id === docId ? { ...item, collectionTag } : item);
                await cacheService.set(CACHE_KEYS.OUTFIT_HISTORY, updated);
