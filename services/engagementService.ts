...
interface NotificationPayload {
  title: string;
  body?: string;
  delayMs?: number; // For scheduled notifications
}

const NOTIFICATION_CONFIG: Record<EngagementEventKey, { 
 ...
 */
function shouldNotify(key: EngagementEventKey): { allowed: boolean; reason?: string } {
 ...
 */
export async function checkDormantAndNotify(): Promise<void> {
  const isNative = Capacitor.isNativePlatform();
  if (!isNative) return; // Web doesn't need dormant check
