...
import { Sparkles, ArrowRight, LogOut, Moon, Sun, Bell, Crown, X, Palette, ChevronRight, Menu, Tag, Zap, Camera, Star, Loader2, Quote, Lock, Clock3, Check, Info, WifiOff } from 'lucide-react';
...
import { checkGenerateLimit, markBonusUsed } from '../services/softWall';
...
const SettingsModal = ({ isOpen, onClose, user, updateUser, onLogout, onOpenHistory, onOpenPremium, onOpenGuide, onNotify, }) => {
    if (!isOpen)
        return null;
...
export const Dashboard = ({ user, onLogout, updateUser }) => {
    ...
    const [checklistState, setChecklistState] = useState(() => onboardingLocal.getState());
    const [premiumMeta, setPremiumMeta] = useState(null);
    const { isAnyOpen: isModalOpen } = useModalStack();
    // Swipe navigation between main views (after state declarations)
    const mainViews = ['home', 'wardrobe', 'profile'];
    ...
    const isStepDone = (id) => checklistState.completedSteps.includes(id);
    const markStep = (id) => {
        if (isStepDone(id))
            return;
    ...
    const handleGenerateClick = async () => {
        ...
        try {
            ...
            if (res) {
                ...
                setToast({ type: 'info', title: t('dashboard.alerts.notFound') });
        }
        catch (e) {
            console.error(e);
