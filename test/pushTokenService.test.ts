...
describe('pushTokenService', () => {
  ...
  describe('upsertToken', () => {
    it('should throw error for invalid platform', async () => {
      await expect(
        upsertToken({ token: 'test_token', platform: 'web' as any })
      ).rejects.toThrow('Invalid platform: web');
    });
    ...
    it('should upsert token with merge on new write', async () => {
      ...
      (auth.currentUser as any) = { uid: userId };
    ...
    it('should handle upsert errors gracefully', async () => {
      ...
      await expect(
        upsertToken({ token: 'bad_token', platform: 'ios' })
  ...
  describe('disableToken', () => {
    it('should throw error for invalid platform', async () => {
      await expect(
        disableToken({ platform: 'web' as any })
      ).rejects.toThrow('Invalid platform: web');
    });
