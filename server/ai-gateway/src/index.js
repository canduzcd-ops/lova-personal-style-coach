...
function withCors(headers = {}, status = 200, body) {
...
async function handleAnalyzeImage(ai, base64Image) {
    const prompt = `
...
async function handleRateOutfit(ai, base64Image) {
    const prompt = `
...
async function handleAnalyzeFace(ai, base64Image) {
    const prompt = `
...
async function handleGenerateOutfit(ai, user, wardrobe) {
...
async function handlePersonalizedTips(ai, user, wardrobe) {
    const userStyles = user.styles.join(", ");
        ...
        try {
            switch (url.pathname) {
                case "/analyzeImage": {
                    ...
                    if (!base64Image)
                        return jsonResponse({ error: "base64Image required" }, 400);
                ...
                case "/generateOutfitFromWardrobe": {
                    ...
                    if (!user || !wardrobe)
                        return jsonResponse({ error: "user and wardrobe required" }, 400);
                    const result = await handleGenerateOutfit(ai, user, wardrobe);
                    return jsonResponse({ result });
                }
                case "/generatePersonalizedTips": {
                    ...
                    if (!user || !wardrobe)
                        return jsonResponse({ error: "user and wardrobe required" }, 400);
                    const result = await handlePersonalizedTips(ai, user, wardrobe);
                    return jsonResponse({ result });
                }
                default:
